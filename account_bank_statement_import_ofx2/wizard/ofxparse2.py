                ...
                try:
                    ...
                    if cls.fail_fast:
                        raise OfxParserException("Empty %s balance\
                            " % bal_type_string)
            if False and hasattr(dtasof_tag, "contents"):       # DONT TRY TO PARSE THE DTASOF FIELD (invalid from CommBank eg <DTASOF>20190144014844)
                try:
    ...
    def parseTransaction(cls, txn_ofx):
        ...
        if hasattr(id_tag, "contents"):
            try:
                ...
                if transaction.id == '':    # fill in a dummy FITID if none is provided
                    transaction.id = datetime.datetime.now().strftime('M%Y%m%d%H%M%S%f')
            except IndexError:
